...
object Interaction2 {
  ...
  def availableLayers: Seq[Layer] = {
    val tColorScale: Seq[(Temperature, Color)] = Seq(
    ...
    val tLayer = Layer(Temperatures, tColorScale, 1975 to 2015)

    val dColorScale: Seq[(Temperature, Color)] = Seq(
      (7, Color(0,0,0)),
      (4, Color(255,0,0)),
      (2, Color(255,255,0)),
      (0, Color(255,255,255)),
    ...
    val dLayer = Layer(Deviations, dColorScale, 1975 to 2015)
  ...
  def yearSelection(selectedLayer: Signal[Layer], sliderValue: Signal[Year]): Signal[Year] = {
    val sliderV = sliderValue()
    val bounds = yearBounds(selectedLayer)()

    sliderV match {
      case v if v < bounds.min => Signal(bounds.min)
      case v if v > bounds.max => Signal(bounds.max)
      case v => Signal(v)
    }
  }
